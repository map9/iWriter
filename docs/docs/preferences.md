...
`偏好设置` 用于管理 iWriter 的工作区、文档版本、编辑体验、拼写与语法检查、主题、导入导出、AI 提供方以及应用更新行为。当前界面分为 `工作区`、`文档版本管理`、`编辑器`、`拼写与语法`、`显示`、`打印与 PDF 输出`、`导出`、`AI`、`更新` 九个分组。
...
- 想配置导出目录、Pandoc 路径、LibreOffice 路径与不同格式的输出参数
- 想控制 `.gitignore` 在不同工作区功能中的过滤范围
- 想配置 Git 环境、提交身份、版本保存、同步和差异查看行为
...
- `.gitignore` 联动：可以分别决定 Explorer、跨文件搜索和文件监听是否跳过 `.gitignore` 中的路径；默认都不启用
- 工作区忽略规则：编辑全局默认规则，与项目根目录的 `.iwtignore` 一起生效

## 文档版本管理

文档版本管理分组统一管理 Git 相关行为：

- Git 环境：自动检测系统 Git，或指定 Git 可执行文件路径
- 提交身份：配置全局 `user.name` / `user.email`，也可以为当前文档仓库设置覆盖值
- 无暂存更改时提交：
  - `提交所有更改`：自动执行 Stage All 后提交，包括未跟踪文件
  - `禁用`：要求先手动暂存
  - `每次询问`：点击提交时再确认
- Pull / Fetch：控制 Pull 自动暂存本地修改，以及 Fetch 是否清理失效远程分支
- 差异查看器：设置默认 Split / Inline 布局和行号
- 文档版本视图：控制仓库、Graph 的显示及文件列表布局

完整说明见 [Git 文档版本管理](/docs/source-control)。
...
- 代码块语言列表：可在 `常用语言` 与 `全部支持的语言` 之间切换。默认使用常用语言，让代码块工具栏的语言下拉更短；需要较少见语言时可切换为完整列表。
...
| **LibreOffice 路径** | 自动检测 / 手动指定 | 用于 Office 文档预览。留空时自动检测系统安装位置或 PATH；手动输入可执行文件路径或 LibreOffice 程序目录。Windows 上建议填写 `soffice.exe` 或 `program` 目录的绝对路径 |
...
- 配置可用后，AI Doc Buddy / AI Story Buddy 中的网页研究工具会使用当前可用的搜索服务

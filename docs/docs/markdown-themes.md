...
iWriter 的主题系统让你可以独立控制 Markdown 文档在**屏幕编辑**和**打印/PDF 输出**时的视觉样式。系统内置五套主题，同时支持用户通过 CSS 文件创建自定义主题，App 会自动发现并热更新。
...
跟随当前应用主题的语义颜色，并保留编辑器默认布局与间距。

- **屏幕显示**：切换 Light、Dark 或其他应用主题时，正文、链接、引用、表格与 Alert 会同步使用当前界面颜色
- **代码块**：保持稳定的深色代码配色，避免在不同应用主题下失去可读性
- **打印输出**：固定使用适合白纸的中性浅色排版，不把深色应用背景带入打印
- **适用场景**：希望 Markdown 内容与应用界面自然融合，同时保持可靠打印效果
...
- **链接**：浅蓝色（`#58a6ff`）
- **边框**：暗色分割线（`#30363d`）
- **表格**：延续 GitHub 风格的表格层次，宽表格可在编辑器内横向滚动
...
- **不能与内置主题重名**：`system`、`github`、`github-dark`、`prose`、`novel` 为保留 ID
- **以 `_` 或 `.` 开头的文件夹会被忽略**
...
如果自定义主题的文件夹名与内置主题 ID 相同（`system`、`github`、`github-dark`、`prose`、`novel`），该自定义主题会被跳过，并在加载状态中显示冲突提示。要解决冲突，只需重命名文件夹。

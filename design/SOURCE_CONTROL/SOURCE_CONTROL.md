...
- 空状态：无变更时显示「没有更改」。
- **行布局（对标 VSCode，2026-07-13）**：文件行状态字母**常驻最右**；hover 时操作按钮出现在其**左侧**、互不遮挡（改掉现在"hover 隐藏状态、按钮顶替"）。
- **tree 视图目录聚合色点（2026-07-13）**：目录行右端显示聚合状态点，颜色 = 子树最高严重度（**冲突/删除=红 ▸ 修改/重命名=黄 ▸ 新增/未跟踪=绿**）；hover 时目录级按钮出现在色点左侧。
...
- 创建、重命名、切换、合并、删除、发布分支。
- **选择方式（2026-07-13 定稿，对标 VSCode 但去掉"菜单弹菜单"）**：
  - **切换 / 发布**：存储库 `⋯` 菜单内**直接列出分支、点击执行**（切换=列本地+远程；发布=列未设 upstream 的本地分支）。
  - **合并 / 删除**：存储库 `⋯` 的"合并分支 ▸ / 删除分支 ▸" **子菜单展开列分支**，点选后走预检。
  - **创建 / 重命名**：文本输入对话框（IwDialog，§5.7）。
- **预检 → 说明 → 决策（总纲见 §5.6）**：切换/删除/合并在执行**前**先跑预检：
  - **切换** `preflightCheckout`：工作区是否干净。有更改 → 原生 messagebox 三选一（贮藏并切换 / 迁移更改 `checkout -m` / 放弃并强制切换 `-f`）；干净 → 直接切。
  - **删除** `preflightDeleteBranch`（三查）：① 有未推送到远程的提交；② 未并入 main；③ 有其他本地分支基于它派生（`branch --contains`）。任一命中 → 原生 messagebox 列问题 + `强制删除`（该框即唯一确认）；全过 → "您确认要删除吗？" + `删除`。**候选排除 main / master / 当前分支**（不进列表）——修掉现在只排除当前分支、可删 main 的缺口。
  - **合并** `preflightMerge`：能否快进（`merge-base --is-ancestor`）。非快进 → 提示会产生合并提交、冲突进 Merge Changes。
- 废除旧的"先删/切 → 失败 → 再弹强制"反应式路径。
...
### F14 · Git 错误反馈（P0，2026-07-13 重订）
- **总纲见 §5.6「预检 → 说明 → 决策」**：能事前预检的问题（切换脏树、删除分支三查、合并快进）一律在执行前用原生 messagebox 说明并让用户决策，绝不"先做后警"。
- **git 报错一律原生 messagebox**——**工程错误对话框 `GitErrorResolutionDialog` 退役**：
  - **已识别的失败**（`remote-auth` / `network` / `remote-non-fast-forward`）：大白话说明 + 恢复动作（落后 → `拉取并重试`）。
  - **未识别的失败**（`unknown`，程序无法翻译）：**把 git 原始输出作为提示正文**，按钮 `关闭 / 复制错误信息 / 重试`。
- **不设"技术详情"折叠、不翻译 git 报错**：能识别的给大白话，识别不了的直接给 git 原文可复制。完整 stderr 仍进日志。
- **纯破坏性确认**（放弃更改、git clean 等）继续用原生 `showMessageBox`。输入校验（分支名/URL）显示在字段旁。
...
- **待实施（2026-07-13 UI 交互梳理定稿，见 §5.6/§5.7 与 `ui/`）**：① 容器 `⋯` = 远程/贮藏/标签/视图**扁平分组**（不再多层 submenu 套娃——这是"菜单不如上一版"的根因）；② 存储库 `⋯` = 分支操作 + 切换分支：切换/发布菜单内列分支点击，合并/删除**子菜单列分支**，删除排除 main/master/当前；③ 贮藏列表/标签列表/管理远程改**列表管理对话框**（替代嵌套原生菜单）；④ 切换/删除/合并接入**预检**；⑤ 变更行状态常驻 + 按钮左移、tree 目录聚合色点；⑥ 全部错误改原生 messagebox、`GitErrorResolutionDialog` 退役。
...
### 5.6 交互总纲 · 预检 → 说明 → 决策（2026-07-13）

> **绝不"先做后警"。** 任何可能出问题的 git 操作（切换/删除/合并分支、放弃、pull/sync…），执行**前**先跑预检：
> - **无问题** → 非破坏性直接执行；破坏性（放弃/删除）弹一次防误点确认。
> - **有问题** → 弹原生 messagebox 把问题用大白话列成信息 + `取消` + `强制/继续`，按用户选择再操作。
>
> 该框双重作用：① 防误点提醒 ② 问题前置说明（避免操作完才提示）。真实可预检的操作三个：**切换 / 删除 / 合并分支**——新增主进程 `preflightCheckout / preflightDeleteBranch / preflightMerge`，返回 `{ ok, problems[], canForce }`。废除反应式的事后错误分类路径。

### 5.7 对话框三形态（2026-07-13）

全 SCM 只有三种对话框，分支选择等一律走菜单（§5.5）：
1. **IwDialog（工程外壳 · 仅文本输入）**——创建/重命名分支、克隆、创建标签、贮藏、添加远程、配置身份。统一一个组件，外形对齐打印/偏好对话框（含关闭 ✕ + 页脚 + Esc），替代现有 5 份复制粘贴内联模态。
2. **列表管理对话框**——贮藏列表（应用/弹出/丢弃）、标签列表（删除 + 创建）、管理远程（移除 + 添加）；列条目 + 每行操作，替代原来的嵌套原生菜单；破坏性动作点了再原生确认。
3. **原生 showMessageBox**——一切确认、预检结果、失败提示（含 `unknown` 显示 git 原文 + 复制）。

设计稿见 `ui/dialogs.html`；面板/菜单/行状态/空态见 `ui/panel.html`。

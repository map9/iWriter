...
> 状态：v0.7（2026-07-13，M1–M7 主线完成；错误反馈与菜单信息架构已实现，见 F14/§5.5/`ui/`）
...
- **删除分支**：选择目标分支后立即尝试安全删除；“分支含未合并提交”是预期状态，不作为原始 Git 错误展示。界面解释风险后，用户可取消或直接选择“强制删除分支”；该风险对话框即为唯一确认，只有该明确选择才执行 `git branch -D`。
...
- **确认的职责边界**：纯破坏性确认（放弃、中止合并等）继续使用原生 `showMessageBox`。安全删除分支不额外确认；未合并分支的项目内风险对话框同时负责解释与强制删除确认，避免重复弹窗。其他需要解释、恢复操作或技术详情的失败反馈使用工程 dialog 外壳。

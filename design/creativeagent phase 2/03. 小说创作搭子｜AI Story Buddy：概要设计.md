...
| `.iwriter/status.md` | 可选 | 仅 agent | 否 | 派生状态摘要缓存，纯缓存，详见 1.4 |
...
**统辖区间定界规则**：一个 beat 哨兵统辖的正文区间 = 从该哨兵行之后起，到**下一个 beat 哨兵行**（不含）或文件末尾为止——哨兵之间不存在无归属正文。场景边界不设独立标记：场景序号变化的哨兵即场景切换点，"场景-N 的正文区间"= 其全部 beat 统辖区间之并。SA02 重写某 beat、SS11 逐场景比对均按此规则取区间；哨兵之前（章首第一个哨兵之前）的正文若存在，视为该章的无 beat 引子，归属场景-1。

beat 哨兵是创作脚手架，不是作品内容：格式固定、可正则识别，**定稿归档与任何导出产物必须剥离哨兵**，剥离规则属格式转换层（实施阶段落地）。哨兵在编辑器中对作者可见，是有意暴露的创作结构（作者可直接修改提纲本身），渲染上可折叠/弱化显示，留实施阶段决定（见 §9）。
...
- `.iwriter/status.md` = **派生状态摘要缓存**，只在任务需要全局态势时按需读取或重建，不作为会话启动必读项，不进 git；
...
SS01-SS05、SS15 是 schema 参考技能，字段定义的权威来源在《04.2 对象模型 详细设计》；SS06-SS14 技法/核查技能的原则与正反例详见《04.3 Agent+Skills 详细设计》§2；SS16-SS17 是 S10 导入编排下的流程子技能（整备/提炼，见 §6 与《04.3》§2）。概要级不变量：
...
- `confirm_writing_plan(plan, rationale, impact?, alternatives?)`：§5 写作审批模型的**授权开关**——批准即开启"块级自动累积"的写作会话（§5.3）；子 Agent 内没有对话通道，因此由 A00 在委派 SA02 前调用，批准的计划随 brief 下达，同时是 SS11 忠实度核查的比对基准；approve/edit/reject 中 edit 即"改完即契约"。使用有条件：新场景/新章节、超过一段的改写、涉及已确认设定的修改、结构性调整才触发；局部润色与小范围修订跳过，直接进入块级编辑 + 章节级审批。
...
机制结论：deepagents 技能加载以**目录为挂载单元**、多个源之间同名 last-wins；子 Agent 不继承主 Agent 技能（框架自动提供的 general-purpose 例外——S09 与 S10 提炼批次（SS17）通用委派的承重假设，实施前优先核验）；项目级 skills 目录 `{workspace}/.iwriter/skills` 固定挂各源列表末位，last-wins 即 FR-9.3"项目级自定义 > 内置默认"优先级裁决的机制实现。creative 实例结论：主技能按执行途径分组（main/ideation/prose/review/research/delegated，delegated 挂 A00 供 general-purpose 继承），SS01-SS17 全部进 `creative/reference/` 单一共享池（SS 交叉引用度高，零拷贝）；技能目录名 = 技能 name（常规 kebab-case，如 `scene-to-prose`，不带 ID 前缀——S/SS 编号是设计文档层的索引，不进入目录名，编号与技能的对应关系见 §2.2 总表）。完整目录树、挂载矩阵与现有 29 个旧技能的收编/下线映射见《04.3》§3。
...
- **Skills 尚未验证**：§2 的 11 个主 skill + 17 个子 skill 目前只是设计结论，还没有走过"无此 skill 的基线场景 → 有此 skill 的场景"这样的验证循环（issues#7 的教训）。验证方案的完整设计见 §10，实施阶段按该方案执行，不能假设设计阶段的推理就等于实际有效。

...
设计要做的事情，是把"**从灵感到一部小说完稿**"这个完整创作过程中涉及的全部场景与流程（含每个环节的产出物），忠实地复刻到这个框架已有的四类积木上：**main agent（A00）+ sub agent（专用 SA02 Writer / SA03 Reviewer + 框架原生 general-purpose；原 SA01 Explorer、SA04 Researcher 已退役）+ skills（S01-S11 主技能、SS01-SS17 子技能）+ tools**。设计的主体工作不是发明一套新的架构概念，而是回答"这个场景对应哪段流程、这段流程产出什么、该由哪类积木承担"——这个映射关系本身才是设计内容，具体映射结果见 §2；§2.2 附有从灵感到完稿的**小说创作流程图**，展示这个流程如何落到 A00/SA/S/SS 各角色上，那张是流程图，不是这里要给的整体架构图。
...
block:layers["本设计：分层实例化（自下而上）"]
  ...
  L3["Skills + subAgent 层：S01-S11 / SS01-SS17 + A00/SA02-SA03 + general-purpose（§2.2-2.6）"]
...
| `outline/ch{NNN}-outline.md` | 懒创建，但**写该章前置必需** | 作者+agent | 是 | 提纲价值的关键载体；一旦作品分章节，写第 N 章前该章章纲必须先成形并 `confirmed`——事件级总纲不能替代场景级章纲（闸3，见《04.5》§2）。不必一次建全部章纲，但绝不从总纲直接写正文 |
...
- 三类大纲文件（总纲/卷纲/章纲）均有必选 status 字段（draft/confirmed，FR-2.3 落点）；章纲场景三段式：目标（可验证）/ 冲突 / 结果（禁止"顺利达成"）；
...
**后续 prompt / skills 设计注意**：A00、SA02-SA03 的 prompt 与 S01-S11/SS01-SS17 技能文档都必须显式遵守上面的状态边界，不得重新写入"每轮启动读取 status / 全局扫描 / 自动 git diff / 自动重建摘要"这类流程。prompt 应强调：先理解用户意图，再按任务读取最小必要对象；skills 应把当前 domain 的用户级记忆 `~/.iwriter/ai/memory/{domain}/memory.md` 当作协作规则来源，把 `status.md` 当作可选缓存，把 git 当作按需比较/回退/固化工具。作者成熟度、干预强度、风险偏好只能改变解释、澄清、预检与提案方式，不能改变作品事实来源，也不能绕过审批红线。本期 `memory.md` 只读装配，prompt/skills 不得包含任何写入或更新记忆的步骤（§2.13）。任何需要全局态势的技能步骤，都应写成"必要时读取/提议重建/请求确认"，而不是默认执行。
...
按此重审，专用子 Agent 收敛为**两个**：SA02 Writer（写稿高频 + 块级编辑工具面）、SA03 Reviewer（工具层收窄的 workspace 只读 + 独立上下文避免确认偏差；合并"好不好"编辑评审与"对不对"一致性校对为一个只读评审执行体，详见下段）。**原设想的 SA04 Researcher（纯共用承包商）也退役**（对齐代码）：框架自带的 general-purpose 已能承接"隔离大段查证上下文"的委派，再设一个纯承包商无区分度（判据同下述 WritingStyle）——S08 web-research 因此收归 **A00 编排**（简单查证 A00 直接用 web 工具做、复杂查证经 general-purpose 委派，交付件落会话级 `/large_tool_results/`）。**原 SA01 Explorer 退役**：发散（生成有区分度的方向）是 A00 对话里的思考动作、不是独立执行体——只有"上下文隔离/固定契约"才构成设专用子 Agent 的理由，而发散既无需更窄工具面、也无需隔离上下文（方向的价值在被作者当场比较，不在落盘），因此降为 A00 载 `direction-ideation` 技能直接跑；探索到的"可行性查证"（史实/专业/世界规则）走 web-research（A00 直接编排或 general-purpose 委派，见本节上文），不另设创意型分身。专用子 Agent 本身也不写死在代码里，而是以声明式定义文件动态装配（见 §2.10）——"身份定位、工具范围、输入输出契约、安全红线"这四类 prompt 内容恰好就是一个小 markdown 文件的体量。曾设想的 WritingStyle 专用子 Agent 不满足任何专用化判据：它不需要更窄的强制工具面（写入 `styles/` 之外的路径已有 `interruptOn` 审批兜底）、不需要不同模型、频率极低（一个项目通常只提炼一两次风格）；它唯一的真实诉求是隔离大段范文文本，通用子 Agent 委派即可满足。因此风格提炼定为 **S09 技能 + 通用子 Agent 委派**，不设专用执行体。反向提炼同样不设专用执行体，但执行途径不同：S10 定为 **A00 直接执行的导入编排主技能**——整备阶段（SS16）必须与作者交互确认章节边界，而子 Agent 内没有对话通道；只有其提炼批次（SS17）需要隔离大段正文上下文，经通用子 Agent 委派（§2.5、§6）。

**编辑角色分两半：前端建议（A00 技能）+ 后端评审（`reviewer` 专用只读子 Agent）。** "编辑"是四类职能——**策划**（写什么、值不值得写）、**发展性**（故事成不成立）、**文字**（表达准不准）、**校对**（文本对不对）。按介入方式分两半：**前端评审 = 构思循环的「比较」拍**（策划 + 发展性建议，对象是创意/设定/大纲/章纲，需与作者来回商量、边议边改）→ **A00 直接执行的比较框架技能**（`idea-review` 创意评审 / `outline-review` 大纲评审，按文档《C》2.3 五段落地；由 `ideation-outline-playbook` playbook 的元循环驱动，输出形状写进技能自身、不再寄存在 A00 prompt）——**按发展阶段整体评审、不按对象拆**（设定/人物/场景是 `outline-review` 内部维度）；**后端评审**（发展性/文字/校对，对象是正文，可离线、只读、产检查文件）→ **`reviewer` 专用只读子 Agent**（原 SA03 ConsistencyChecker 升格）。

后端设专用子 Agent 的理由：判据 (a) 工具层强制的 **workspace 只读**（评审只回意见/方向、绝不改稿，通用委派默认带 `write_file`/`edit_file`，只读全靠 prompt 约定）+ 判据 (b) 独立上下文避确认偏差。三套镜头**形状同一**（只读、按参照物评、简报 + 落盘检查文件交付、绝不改稿），差别只在镜头，由 brief `scenario` 选载——正是 §2.1 落位第 4 条（conditional）= subSkill。三个评审任务：**发展性·好不好**（`developmental-review`，`scope` chapter/manuscript）/ **文字·文字精修**（`line-editing-review`）/ **校对·对不对**（`consistency-review`，忠实度 + 硬一致两阶段两 verdict），可多选 = 全视角。

关键设计点（完整契约在《04.3》§1 S06）：agent.md 正文承载**公共评审协议 + 统一输出契约**（一套五类输出 + 四级优先级，只在 agent.md 定义、三镜头不重复），三镜头用同一四段式模板；核心原则**分阶段递减干预**（越近定稿越收方向性干预）；**语法校对归 App 的 proofread（LanguageTool）、不归 reviewer**；权限形态 = 只读子 Agent 通用款（只写 `/large_tool_results/`、workspace 只读靠审批层、不设 blanket write-deny）；多分身并行为后期增强，初始单分身。

前端与后端**零评审技能耦合**（只共用 `creative/common`/`creative/reference` 通用池），故可干净拆分：前端比较（idea/outline-review）只判断预正文对象、A00/writer 只落笔、reviewer 只对正文只读评审——三条边界不糊；发散（direction-ideation）作为「比较」拍内的思考子动作被 A00 调用，不构成独立评审角色。

**元循环 + 阶段化 = A00 运行节律（详见《04.5》§1.0）。** 从主题/创意到场景，**每个创作环节都由「发散·比较·收敛」三拍产出**，收敛结果即产出物、成为下一环节前提，重改产出物 = 重入循环——这是全流程唯一元流程，讨论即创作、成文只是转录。为免 A00 载入跨阶段无关内容，宏阶段拆三个 **playbook 动态载入**（`ideation-outline-playbook` / `drafting-playbook` / `revision-playbook`），A00 常驻只留身份 + 对象模型 + 通用纪律 + 阶段路由，`story-development-flow` 判阶段后载对应 playbook。**三拍三粒度**：发散按分叉类型（`direction-ideation` 可扩展主技能 + 子技能槽）、比较按发展阶段整体评审任务（idea/outline-review，不按对象拆）、收敛按对象（per-object authoring）。落位仍遵《04.5》元循环与本节三层分类，不与之冲突。
...
flowchart LR
  ...
  subgraph 技能层["技能层：主 Skill（每个创作场景一个）"]
    ...
    S02["ideation-outline-playbook（构思循环 playbook：发散·比较·收敛）"]
    S03["S03 worldbuilding-authoring"]
    S04["S04 outline-authoring"]
    S05["S05 scene-to-prose"]
    S06["S06 review（发展性/文字精修/一致性，可多选）"]
  ...
  SA02 --> S05
  SA03 --> S06
  A00 -.->|简单查证直接做/复杂查证委派| S08
  GP --> S09
  S08 -.->|复杂查证 general-purpose 委派| GP
...
- **核心角色（subAgent，主 skill 写进 agent.md；A00 主控写进 system prompt）**：A00 主控（主 skill S01 task-routing 折叠进 A00 system prompt——每轮必用、单一消费者，不作独立挂载 skill）；SA02 Writer（主 skill S05b）；SA03 Reviewer（主 skill S06 review：编辑角色后端三镜头 `developmental-review`（好不好，scope chapter/manuscript）/ `line-editing-review`（文字精修）/ `consistency-review`（对不对），可多选=全视角，workspace 只读，agent.md 承载公共协议 + 统一输出契约，产出 `/large_tool_results/` temp 检查文件）。**S08 web-research 无专用执行体**（原 SA04 Researcher 退役）——由 A00 直接编排（简单查证）或经框架 general-purpose 委派（复杂查证），技能 `common/web-research` 保留。
- **主要角色（挂载的主 skill）**：S03 worldbuilding-authoring、S04 outline-authoring、S05a writing-plan-authoring、S07 restructuring、S09 style-transfer、S10 novel-import、S11 project-bootstrap。
- **纯技能（subSkill，引用不挂载、增量非强制）**：SS01-SS17 全部——由其 owner（核心角色 agent.md 或主要角色 SKILL.md）以相对路径引用；下表"包含的子 skills"列即引用关系，不代表独立挂载单元。

因此下表"分类"列中的"主 skill / 子 skill"是沿用的文档索引词：核心角色的主 skill（S01/S02/S05b/S06）实际折叠在 agent.md（A00 的 S01 折叠进 system prompt），S08 无专用执行体、由 A00 编排（`common/web-research` 技能挂载），其余主 skill 为主要角色挂载单元，"子 skill"一律是 subSkill。
...
| SA03 | Reviewer子agent | Reviewer | 编辑角色后端只读评审：发展性·好不好（scope chapter/manuscript）/ 文字精修 / 校对·对不对，可多选=全视角；产出简报+`/large_tool_results/` 检查文件（五类输出+四级优先级） | A00 判定为评审类并委派（写后自动跑 developmental@chapter=模式A；作者触发=模式B） | S06（含镜头 developmental-review、line-editing-review、consistency-review） |
| —(查证) | 无专用执行体 | web-research | 查证现实细节/专业背景 | A00 直接编排（简单）或 general-purpose 委派（复杂），原 SA04 退役 | S08 |
| S01 | 主skill（折叠进 A00 prompt） | task-routing | 判定场景类别+委派决策 | 每轮 A00 处理前，内联在 system prompt（非独立挂载 skill） | 无（顶层路由） |
| S02 | 主skill（A00 直接执行·构思 playbook） | ideation-outline-playbook | 构思阶段元循环指挥（发散·比较·收敛 + 相互讨论平衡阀 + 阶段过渡），路由到发散/比较/收敛技能 | 构思类（主题/设定/大纲环节） | direction-ideation、idea-review、outline-review（收敛转 S03/S04/S11） |
| —(比较) | 主skill | idea-review | 创意评审（策划编辑读）：值不值得写、哪个方向最值得发展；输出=判断/方向/方案/追问/验证任务（文档 C 2.3） | 创意/概念环节的「比较」拍 | SS09、SS06、SS07 |
| —(比较) | 主skill | outline-review | 大纲评审（发展性编辑读·整体）：设定/人物/世界观/叙事策略/大纲/章纲一体评，故事成不成立 | 大纲及其上游环节的「比较」拍 | SS08、SS06、SS07、SS09 |
...
| S06 | 主skill | review（reviewer 公共协议+统一输出契约，折叠进 agent.md） | 编辑角色后端只读评审三镜头：developmental-review（好不好，scope chapter/manuscript）/ line-editing-review（文字精修）/ consistency-review（对不对=忠实度 SS11+硬一致 SS12 两阶段）；按 `scenario` 选载、可多选，产出简报+`/large_tool_results/` 检查文件 | 评审类（模式A 写后自动 developmental@chapter、模式B 作者触发任一任务） | developmental-review、line-editing-review、consistency-review（含 SS11、SS12）、SS03、SS04、SS02 |
...
| SS01 | 子skill | project-template | 作品对象字段定义 | 读写 `project.md` | 无 |
| SS02 | 子skill | worldbuilding-schema | 设定对象字段定义 | 读写 `worldbuilding/*.md` | 无 |
| SS03 | 子skill | characters-template | 角色对象字段定义 | 读写 `characters/*.md` | 无 |
...
| SS11 | 子skill | fidelity-check | 忠实度核查（consistency-review 镜头第一阶段） | 评审场景含对不对 | REQUIRED BACKGROUND: SS04、SS03 |
| SS12 | 子skill | prose-quality-check | 质量核查（常识/POV/行为一致/时序一致/伏笔/风格；consistency-review 镜头第二阶段） | 评审场景含对不对 | REQUIRED BACKGROUND: SS03、SS02、(SS05) |
| —(镜头) | 子skill | developmental-review | 发展性·好不好镜头：故事成不成立（六维强度=故事机制/人物/叙事 强），`scope: manuscript` 另出评估信/修订路线图/保留清单；只给方向不写细则 | `scenario` 含 developmental 时选载（模式A 只此镜头/模式B 可含） | 共用 restraint、结构/主题范式（`creative/common`） |
| —(镜头) | 子skill | line-editing-review | 文字·文字精修镜头：语言级（表达实现 强），对话/表现力/节奏/冗余/匠气 + 少量代表性改写，不重开结构 | `scenario` 含 line 时选载（模式B） | 共用 restraint（`creative/common`） |
| —(镜头) | 子skill | consistency-review | 校对·对不对镜头 = 忠实度（SS11）+ 硬一致（SS12，含时间/空间连续性）两阶段，两 verdict 分开；不含语法校对（proofread 负责） | `scenario` 含 consistency 时选载（模式B、commit 提醒） | SS11、SS12 |
| SS13 | 子skill（发散·可扩展包，A00 载） | direction-ideation | 否定显而易见版本 + 承重轴分叉 + 区分测试；主技能 + 分叉类型子技能槽（结局/引擎/人物入口/分支/结构转点/叙事策略，懒生长） | 「发散」拍：作者要看不同走法时 | 子技能槽（增量） |
| SS14 | 子skill | author-style-application | 应用已确认风格 | 写稿/审校且风格已存在 | REQUIRED BACKGROUND: SS05 |
| SS15 | 子skill | others-schema | 素材/未决问题/创作日志/审校问题清单对象字段定义 | 读写 `materials/`、`process/` | 无 |
...
| --- | --- | --- | --- | --- |
| 构思类（主题/创意环节） | 前期筹备 | S02 ideation-outline-playbook（playbook）→ idea-review（比较）/ direction-ideation（发散）/ S11（收敛） | SS13、SS09、SS06、SS07 | 构思模式（发散·比较·收敛） |
| 设定类 | 前期筹备 | S02 playbook → outline-review（比较）/ direction-ideation（发散）/ S03（收敛） | SS02、SS03、SS06、SS09 | 构思模式 |
| 大纲类 | 前期筹备 | S02 playbook → outline-review（比较）/ direction-ideation（发散）/ S04（收敛） | SS04、SS07、SS08、SS09 | 构思模式 |
| 写稿类 | 初稿写作、二轮精修 | S05 | SS03、SS04、SS10、SS06、(SS14 条件) | 写稿模式 |
| 评审类 | 写稿后（模式A 自动 developmental@chapter）/ 审校期（模式B 作者触发发展性/文字精修/一致性/全视角，含局部大修与全稿定稿） | S06 review（reviewer 子agent） | developmental-review、line-editing-review、consistency-review（=SS11+SS12）、SS03、SS04、SS02、(SS05 条件) | reviewer 子agent 委派（workspace 只读、隔离上下文、只回意见/落 temp 检查文件不改稿；全稿级可加读 `status.md`） |
...
| 初始化类 | 前期筹备（进入之前，从零新建工程） | S11 | SS01（骨架创建另引 SS02/SS03/SS04 字段标题） | 最小装配（空工程无既有对象可读；premise 引导可转构思循环（ideation-outline-playbook）） |
...
| S02（ideation-outline-playbook） | 收敛写 `project.md`/`worldbuilding/`/`characters/`/`outline/`（经 S03/S04/S11）；发散可选暂存 `exploration/` | 块级读写 + create_document | SS13、idea-review、outline-review |
...
| S06 review | 读 `manuscript/`、`outline/`、`characters/`、`worldbuilding/`；需要全局摘要时可读 `.iwriter/status.md`；写 `/large_tool_results/review-*.md`（检查文件，workspace 只读） | get_blocks（按核查颗粒度）、find_references、write_file（仅 `/large_tool_results/`） | developmental-review、line-editing-review、consistency-review（=SS11+SS12） |
...
| SS17 | 读 `manuscript/` 章节区间；写 `outline/`（标 draft）、`exploration/`、`/large_tool_results/`（分批中间证据） | get_blocks、create_document、edit_block、write_file（仅虚拟区） | SS04、SS03、SS02、SS07 |

### 2.5 主 Skill 详述（S01-S11）（详细设计 → 04.3）

十一个主 skill 的逐步执行流程、brief 契约、状态回报与红线详见《04.3 小说创作搭子｜AI  Story Buddy：Agent+Skills 详细设计》§1。概要级骨架结论：

- **S01 task-routing**：A00 每轮强制执行的轻量路由——作者显式指定模式或流程时直接采用、跳过判定信号（FR-2.5，审批红线不放宽）；否则依次看协作记忆（只影响方式不提供事实）、对象路径前缀、`project.md` 版本倾向、用词特征、落地目标风险；信号矛盾时一次最小澄清，不默认选高风险分支；判定后决定直接执行 / 委派 SA02 Writer·SA03 Reviewer / 通用委派（S09 风格提炼、S08 复杂查证；S10 由 A00 直接编排，其提炼批次经通用委派）；工作区缺 `project.md` → 提议初始化（S11，只提议不自动执行）；`manuscript/` 非空而设定单薄 → 提议 S10（只提议不自动执行）；"我写到哪了"类进度恢复请求 → 定点读取（`manuscript/` 目录列表 + 末章章纲状态 + 未闭合会话登记 + `open-questions.md` 待决策条目，`status.md` 存在时可作快速提示但须核对）汇总回答并提议下一步，不做全局扫描；"记一下"类请求 → 按 SS15 直接追加碎片，不展开流程；写稿委派返回前提缺口 → 切回构思补齐后继续（FR-2.4）。**阶段跃迁的就绪门槛**：判断"该不该进下一阶段"（设定→总纲、总纲→章纲、章纲→写章节）时读《04.5》创作流程与门槛——上游不就绪则摆缺口、提议先补，agent 不自越闸，作者显式越闸给一次质量提示（前提缺口回补机制从写稿推广到所有跃迁）。候选内容作者确认前不得进入正式对象路径。**"每轮必用"的保证机制**：S01 是 A00 的主技能（每轮必用、单一消费者），按 §2.1 落位法则**直接折叠进 A00 system prompt 正文**（"Routing (every turn)"节），不作独立挂载 skill——由此天然常驻、无"是否已加载"问题，取代了旧的"每轮第一个动作 = 加载 S01 skill 正文 + 轨迹审计"机制（旧机制曾靠技能中间件按需加载 + prompt 顶层强制指令 + §10 轨迹审计三者兜住"skill 有没有被加载"，折叠进 prompt 后此顾虑消失）。即便模型路由判断失误，损害仍被审批体系兜底为"低效"而非"越权"（候选不进正式对象、写作过 `confirm_writing_plan`）。
- **S02（ideation-outline-playbook，A00 直接执行）**：构思阶段的元循环 playbook。每轮先定位"在哪个环节（主题/设定/大纲）× 哪一拍（发散/比较/收敛）"，再按拍执行——发散载 `direction-ideation`（否定显而易见版本、承重轴分叉、只描述差异不评优劣）；比较载 `idea-review`/`outline-review`（按发展阶段整体评审、给判断/方向/方案/验证任务）；收敛经 S11/S03/S04 转录进正式对象。相互讨论平衡阀：主动抛框架点、不代作者裁决、每轮只展开 2–4 个最重要的点其余挂开放待决。候选未确认不进正式对象。
...
- **S06 review（SA03 Reviewer）**：编辑角色后端 workspace 只读评审，brief `scenario` 决定跑哪个任务、可多选=全视角：**发展性·好不好**（`developmental-review`，scope 章/全稿）、**文字·文字精修**（`line-editing-review`）、**校对·对不对**（`consistency-review`，忠实度+硬一致两阶段）；核心原则分阶段递减干预，语法校对归 App 的 proofread。**两种模式**：模式A（写后自动 developmental@chapter、一轮修订、临时不落盘）/ 模式B（作者触发任一任务，可提升进 `process/review-findings.md`）。统一输出契约（五类输出 + 四级优先级）、六维强度、分批与镜头正文详见《04.3》§1 S06。
...
- **S08（A00 编排，无专用子 Agent）**：证据与解读分离，标注来源、置信度与信息缺口；交付件写入**会话级 `/large_tool_results/`**（免审批的内部虚拟区，见《04.1》§6），**不写 `exploration/`**——研究结果是中间原料，要留的内容由 A00 读取后提炼、另行写入正式对象。**执行路径**：简单查证 A00 直接用 `web_search`/`fetch_url` 完成；复杂查证经框架 general-purpose 委派（继承 A00 技能集，含 `common/web-research`，回传 `{path, summary}`）。原 SA04 Researcher 专用承包商退役（对齐代码，理由见 §2.5）。
- **S09（A00 经通用子 agent 委派）**：风格提炼只写 `styles/`，只依据范文不臆造。
- **S10（A00 直接执行的导入编排）**：四段编排——①判定入口：作者带整本单文件（txt/docx/md）→ 从整备开始，`manuscript/` 已有规范化章节 → 跳过整备直达提炼；②整备（SS16）：LLM 只判章节边界（`import_manuscript` dry-run 产候选清单 + 边界附近抽样核对，不读全稿），边界清单经作者确认后由工具 execute 机械完成转换/拆章/命名/噪声剥离——**整备不得由模型逐字搬运正文**；③提炼（SS17，经通用子 agent 委派）：超单批预算时按 §2.14 分批（5-8 章/批），逐批章纲草稿落盘 + 证据落 `/large_tool_results/`；④聚合委派反推总纲、合并候选。结构层写 `outline/` 标 draft，角色/设定层只进 `exploration/`，无证据字段留空标注缺口。
- **S11 project-bootstrap（A00 直接执行）**：从零初始化小说工程（FR-1.6）——对话式引导收集 `project.md` 必选字段（SS01），故事核心句四要素（主角/目标/阻碍/赌注）必须成立，不成立时引导补齐或转构思循环，不代作者虚构；工作区无 git 仓库时调用 `git_init` 建立版本基线（经 interruptOn 审批，非 host 静默创建）；写入 `project.md` 并创建必需对象骨架（`worldbuilding.md`/`characters.md`/`master-outline.md`，大纲标 `status: draft`），均经审批；完成后建议首次 `git_commit` 固化基线。`project.md` 日常维护与 §4 版本推进建议流程同属本技能。初始化必须经作者确认，不得静默执行。

### 2.6 子 Skill 详述（SS01-SS17 + 评审镜头）（详细设计 → 04.3）

SS01-SS17 均为 **subSkill（纯技能，§2.1）**：不单独挂载、不暴露为可委派 skill，由其 owner（核心角色 agent.md 或主要角色 SKILL.md）以相对路径引用，且**按 §10 验证结论增量增删、非强制目录**（能被一份粗粒度方向覆盖的可不独立成文）。分工：SS01-SS05、SS15 是 schema 参考，字段定义的权威来源在《04.2 对象模型 详细设计》；SS06-SS14 技法/核查的原则与正反例详见《04.3 Agent+Skills 详细设计》§2；SS16-SS17 是 S10 导入编排下的流程子技能（整备/提炼，见 §6 与《04.3》§2）。此外 **reviewer 的三套评审镜头也是 subSkill**（按 brief `scenario` 选载）：`developmental-review`（好不好，scope chapter/manuscript）、`line-editing-review`（文字精修）、`consistency-review`（对不对，忠实度 SS11 + 硬一致 SS12 两阶段）。概要级不变量：
...
| web（A00 挂载，general-purpose 继承） | web_search、fetch_url | S08 查证；写交付件到会话级 `/large_tool_results/`（策略层免审批）——A00 直接查证或复杂查证经 general-purpose 委派，无专用 researcher |
...
机制结论：deepagents 技能加载以**目录为挂载单元**、多个源之间同名 last-wins；子 Agent 不继承主 Agent 技能（框架自动提供的 general-purpose 例外——S09 风格提炼、S08 复杂查证、S10 提炼批次（SS17）通用委派的承重假设，实施前优先核验）；项目级 skills 目录 `{workspace}/.iwriter/skills` 固定挂各源列表末位，last-wins 即 FR-9.3"项目级自定义 > 内置默认"优先级裁决的机制实现。creative 实例结论（按 §2.1 三层分类）：**核心角色的主 skill（S02/S05b/S06/S08）折叠进各自 `subagents/{name}/agent.md`，不在 skills/ 下单独挂载**；**主要角色的主 skill 挂载**（`creative/main`：S01/S03/S04/S05a/S07/S10/S11；`creative/delegated`：S09，挂 A00 供 general-purpose 继承）；**评审镜头子技能挂 reviewer 专属池**（`creative/review`：`developmental-review` / `line-editing-review` / `consistency-review` 三镜头，按 brief `scenario` 选载）；**subSkill 按共用面分四处**（§2.1）：跨顶层角色共用的创作范式（`restraint`、场景/人物/结构/主题 = SS06-SS09）在 `creative/common`（writer 写时用、reviewer 审时用、A00 大纲/建议时用，单一真源）；**工程空间/小说对象定义**（对象 schema + 创作流程/门槛 = SS01-SS05/SS15/story-development-flow）在 `creative/reference`——**只放对象/规格，非杂货铺**；角色专属单件在 `creative/{role}`（writer 的 SS10 正向文笔、reviewer 的三镜头）；**仅一个角色内多子部件共用的**在 `creative/{role}/common`（当前暂空，供后续）。技能目录名 = 技能 name（常规 kebab-case，不带 ID 前缀——编号是设计文档层索引，见 §2.2 总表）。完整目录树与挂载矩阵见《04.3》§3。
...
- **批间传递靠落盘中间产物 + A00 编排聚合**，不靠子 Agent 会话延续、批与批的子 Agent 之间不直接通信：
  - **S10** 逐批产出两类中间产物——章纲草稿直接写 `outline/ch{NNN}-outline.md`（标 `status: draft`，经审批，本身就是交付的一部分）；角色/设定层的行为证据与中间归纳落会话级 `/large_tool_results/`（免审批虚拟区，见《04.1》§6；执行提炼批次（SS17）的通用子 agent 因此开放 `write_file`，**仅限该虚拟区**）。终批完成后 A00 追加一次**聚合委派**：读全部批次中间产物与已落盘章纲草稿（不重读全稿正文），反推总纲结构节点与伏笔总表、合并去重角色/设定候选写 `exploration/`。
  - **S06 review** 每批把详细发现写入 `/large_tool_results/review-batch-{n}.md`（免审批虚拟区），并以固定格式响应文本回传 A00 简报 + 该批文件路径（宏观级结论紧凑，响应文本装得下），回报中单列**"跨批线索"段**（未回收伏笔、未闭合时间线锚点、批末角色状态快照）；A00 把前批跨批线索摘要随下一批 brief 前向下达，终批后读各批 temp 文件跨批聚合，两 verdict 仍分开产出。**reviewer 对 workspace 保持只读**——中间发现只落 `/large_tool_results/`（会话级免审批虚拟区），绝不写工作区对象。
...
**运行时承载：`context-discipline`（共用 skill，单一真源）。** 本节此前只存在于设计文档里——运行期没有任何 prompt/skill 承载它，装配全靠模型每轮临场判断，实测后果是"读不全 / 读爆 / 读错基线"三类错误反复出现（典型：拿旧章纲去审作者刚提交的新定稿，正好违反下面第 2 条"不用旧事实压制新方向"）。按 §2.1 落位法则第 2 条（多角色共用 → 独立成 skill），本节落为一条共用 reference skill，由 A00 / SA02 / SA03 三处按名引用，形态与 `restraint` 同款。它承载：三条优先级原则（原样）、按拍（发散/比较/收敛）的最小对象集、正文整读与全局索引定向取节的读法、以及三条工程性纪律——

- **基线律**：任何比较/评判先声明"以什么为准"；基线 = 最近一次作者确认的东西。
- **引用律**：断言"违反了设定 X"必须引 X 的原文；引用与结论矛盾时以原文为准。（实测事故：agent 引用了世界观原文却推出与原文相反的"违规"结论，据此建议作者改稿。）
- **搜索律**：检索工具是字面量匹配，一次一词；**"没搜到"不能作为结论依据**。这条属工具契约层的临时纪律，工具修复后回收（见《04.4》）。
...
| 块级编辑 | edit/insert/delete_block、replace_range、create_document | approve / edit / reject | **主控自己的编辑**：同一快照的多块修订聚合为一张整批卡一次决策，可逐条覆写（§2.12）；**子 Agent 的编辑**：必在会话内、由策略层自动累积（5.3），会话外由 Stage 2b 直接拒绝 | 按文档正序的聚合块级 diff；应用由引擎逆序执行 |
...
- **会话的划分依据是执行体，不是改动量（修订轮结论，替代原"按链路划分"）**：**凡由子 Agent 执行的正文写入，一律必须在写作会话内**——展开链路与修订链路一视同仁；**主控自己的块级编辑不开会话**，按块级逐个审批。原先按"链路/改动量"划分（修订链路不开会话）在实践中把授权与执行体割裂了：被授权的 SA02 反而每块弹卡、没有基线、失败无法整体回退，而主控自己改走的却是同一条逐块卡路径——两种审批形态混在一条通道上。改判依据：会话机制的本质是"**拿逐块审批换低打扰 + 一个可整体回退的基线**"，而这恰恰是**委派**才需要的（子 Agent 没有对话通道、作者看不见中间过程、失败时需要整体回滚）；主控在对话里改，作者本来就在场，逐块卡不构成摩擦也不损失可回退性。
    
    - **强制点（Stage 2b，`decideDelegatedWriteGate`）**：子 Agent 发起的块级编辑 / `create_document`，若目标不在任何活动授权域内 → **策略层直接拒绝该次调用**，拒绝消息告知主控"先开写作会话再委派"。绝不静默放行，也绝不静默替作者登记授权——授权始终来自作者批准的 `confirm_writing_plan`。这把"正文只能由 SA02 写"从 prompt 红线升级为工具层可执行的约束（对齐 §2.1 判据 (a)：需要在工具层强制的收窄）。
    - **originator 判定**：中断载荷不记录发起者，但主控自己的流式消息记录了——某个 actionRequest 的（工具名, 第 n 次出现）在父消息的 tool_calls 里找不到对应项，即只可能来自子 Agent 内部（`delegatedActionIndices`）。父消息 tool_calls 不可得时**一律不判为委派**（宁可漏判不可错判：错判会拒掉主控的合法编辑）。
    - 修订链路的计划文本可以很短——一句修订意图即可，它授权的是"这一遍改这个文件"，不是重写全书。
...
4. **大纲/设定/角色对象的确认里程碑**：三类大纲 status 从 draft 改为 confirmed、或已确认对象的核心字段（心理三角、禁区、结构节点）发生批准变更落地后，agent 顺带建议 commit；
...
**核心原则**：反向提炼出的全部内容（设定规则、角色心理三角、章纲场景列表、总纲结构节点）都是 agent 对既有正文的解读，不是作者确认过的事实，必须先进 `exploration/` 候选区，或——仅限大纲对象——直接写入 `outline/` 并标 `status: draft`，不能直接写成正式（已确认）对象。通常流程是"设定 → 情节 → 正文"，这里反过来"正文 → 提炼设定/情节 → 作者确认 → 再继续正向迭代"，机制复用 1.2 节已定义的"反向生成"能力，不需要新发明。
...
- **结构层**（置信度最高，建议优先做）：逐章读取正文 → 反推每章场景列表（目标-冲突-结果）→ 归纳总纲结构节点与伏笔总表，写入 `outline/master-outline.md`/`vol{NN}-outline.md`/`ch{NNN}-outline.md`，状态一律标 draft；
...
- **范文是主锚，但绝不是前置条件（修订轮澄清）**：下面这条实测结论不变，但运行时表述必须堵住一个误读——"没有范文就不能写"。`styles/` 缺席时按既有回落路径（角色声音特征 + SS10 范例驱动）**全质量照写**，不得停下、含糊或索要范文。同时补一条与 `restraint` 对称的**正向**面：`restraint` 是硬帽（负向可数），而生成向全是软建议，模型在不确定时必然向"少写"坍缩——实测表现为重大冲击场景直接跳过反应链、作者判为"清汤寡水"。补法遵 §7 的"匹配形式与失败类型"：这是 shaping 类失败，**必须用正例**（新增 `prose-craft-by-example/references/impact-rendering.md`：身体反应先于理解 → 抓错把手 → 具体的、属于这个人的恐惧；配强弱对照，**不给三拍公式、不给条数配额**），并在 `restraint` 里点明"restraint 不等于少写"。同理，"生活细节太干"也是 shaping 失败：解法是**上下文供给**（装配时把人物卡/前文里的关系史材料带进 brief，属 §3 装配层）而非"每场景 ≥2 条"的清单。
...
| FR-15 | 资料查证 | §2.5（S08，A00 编排 / general-purpose 委派） |
...
- **正向样本**：按 04.2 的完整示例体系（"照片"工程）构造的迷你工程——已确认的 `project.md`/设定/角色档案、已确认总纲 + 若干章纲（含 draft 与 confirmed 混布）、5-10 章正文（含 beat 哨兵）、少量候选与碎片。覆盖构思/设定/大纲/写稿/审校/重构类技能的验证。

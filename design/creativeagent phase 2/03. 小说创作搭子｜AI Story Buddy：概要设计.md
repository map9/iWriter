...
- **核心角色（subAgent，主 skill 写进 agent.md）**：A00 主控；SA01 Explorer（主 skill S02）；SA02 Writer（主 skill S05b）；SA03 Reviewer（主 skill S06 review：好不好+对不对+两者三场景，workspace 只读，产出 `/large_tool_results/` temp 检查文件；两套镜头 `editorial-review`/`consistency-review` 按意图选载）；SA04 Researcher（主 skill S08，共享 `common/`）。
- **主要角色（挂载的主 skill）**：S01 task-routing、S03 worldbuilding-authoring、S04 outline-authoring、S05a writing-plan-authoring、S07 restructuring、S09 style-transfer、S10 novel-import、S11 project-bootstrap。

...
**创作流程与阶段门槛**：本层级链（设定/角色 → 总纲 →[卷纲]→ 章纲 → beat 计划 → 正文）不是靠 graph 强制的状态机，而是**默认推进序 + 每次跃迁的就绪门槛**——上游成熟到什么程度才配进下一阶段（如设定能否推导冲突、章纲能否让写手不再现编冲突结果）。门槛是质量守门 + 缺口提议，非硬锁：agent 不自越闸，作者可显式越闸并得到一次质量提示。完整的流程图、五道门槛判据、写章节前置状态机与 status.md 状态账本见《04.5 创作流程与阶段门槛 详细设计》。
...
- `confirm_writing_plan(plan, rationale, impact?, alternatives?)`：§5 写作审批模型的**授权开关**——批准即开启"块级自动累积"的写作会话（§5.3）；子 Agent 内没有对话通道，因此由 A00 在委派 SA02 前调用。批准后 A00 把 beat 写成目标正文文件的哨兵（`> [场景-{N}-节拍-{M}] 核心点`，beat 唯一真源），SA02 从文件读哨兵、只写哨兵下正文——**不经 brief 转述 beat**（子 Agent 被隔离，转述必然脆弱）；SS11 比对基准是正文哨兵 + 已确认章纲。approve/edit/reject 中 edit 即"改完即契约"。使用有条件：新场景/新章节、超过一段的改写、涉及已确认设定的修改、结构性调整才触发；局部润色与小范围修订跳过，直接进入块级编辑 + 章节级审批。

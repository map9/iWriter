...
- **S11 project-bootstrap（A00 直接执行）**：从零初始化小说工程（FR-1.6）——对话式引导收集 `project.md` 必选字段（SS01），故事核心句四要素（主角/目标/阻碍/赌注）必须成立，不成立时引导补齐或转构思循环，不代作者虚构；工作区无 git 仓库时调用 `git_init` 建立版本基线（经 interruptOn 审批，非 host 静默创建）；写入 `project.md` 并创建必需对象骨架（`worldbuilding.md`/`characters.md`/`master-outline.md`，大纲标 `status: 草稿中`），均经审批；完成后建议首次 `git_commit` 固化基线。`project.md` 日常维护与 §4 版本推进建议流程同属本技能。初始化必须经作者确认，不得静默执行。

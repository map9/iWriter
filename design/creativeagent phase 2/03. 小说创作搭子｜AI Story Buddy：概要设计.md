...
按此重审，专用子 Agent 收敛为四个：SA01 Explorer（构思高频 + 产出限定候选区的固定契约）、SA02 Writer（写稿高频 + 块级编辑工具面）、SA03 Reviewer（工具层收窄的 workspace 只读 + 独立上下文避免确认偏差；合并"好不好"编辑评审与"对不对"一致性校对为一个只读评审执行体，详见下段）、SA04 Researcher（纯共用承包商，web 工具面 + 交付件落盘会话级 `/large_tool_results/`）。专用子 Agent 本身也不写死在代码里，而是以声明式定义文件动态装配（见 §2.10）——"身份定位、工具范围、输入输出契约、安全红线"这四类 prompt 内容恰好就是一个小 markdown 文件的体量。曾设想的 WritingStyle 专用子 Agent 不满足任何专用化判据：它不需要更窄的强制工具面（写入 `styles/` 之外的路径已有 `interruptOn` 审批兜底）、不需要不同模型、频率极低（一个项目通常只提炼一两次风格）；它唯一的真实诉求是隔离大段范文文本，通用子 Agent 委派即可满足。因此风格提炼定为 **S09 技能 + 通用子 Agent 委派**，不设专用执行体。反向提炼同样不设专用执行体，但执行途径不同：S10 定为 **A00 直接执行的导入编排主技能**——整备阶段（SS16）必须与作者交互确认章节边界，而子 Agent 内没有对话通道；只有其提炼批次（SS17）需要隔离大段正文上下文，经通用子 Agent 委派（§2.5、§6）。

**编辑角色分两半：前端建议（A00 技能）+ 后端评审（`reviewer` 专用只读子 Agent）。** "编辑"是四类职能——**策划**（写什么、值不值得写）、**发展性**（故事成不成立）、**文字**（表达准不准）、**校对**（文本对不对）。按介入方式分两半：**前端建议**（策划 + 发展性建议，对象是创意/设定/大纲/章纲，需与作者来回商量、边议边改）→ **A00 直接执行的建议技能**（`idea-advisory` / `outline-advisory`，前端定位见 §2.5，本轮只出规格、不落地）；**后端评审**（发展性/文字/校对，对象是正文，可离线、只读、产检查文件）→ **`reviewer` 专用只读子 Agent**（原 SA03 ConsistencyChecker 升格）。
...
前端与后端**零评审技能耦合**（只共用 `creative/common`/`creative/reference` 通用池），故可干净拆分：explorer 只生成、前端建议只判断、A00/writer 只落笔、reviewer 只对正文只读评审——四条边界不糊。
...
- **核心角色（subAgent，主 skill 写进 agent.md；A00 主控写进 system prompt）**：A00 主控（主 skill S01 task-routing 折叠进 A00 system prompt——每轮必用、单一消费者，不作独立挂载 skill）；SA01 Explorer（主 skill S02）；SA02 Writer（主 skill S05b）；SA03 Reviewer（主 skill S06 review：编辑角色后端三镜头 `developmental-review`（好不好，scope chapter/manuscript）/ `line-editing-review`（文字精修）/ `consistency-review`（对不对），可多选=全视角，workspace 只读，agent.md 承载公共协议 + 统一输出契约，产出 `/large_tool_results/` temp 检查文件）；SA04 Researcher（主 skill S08，共享 `common/`）。

...
**"好不好"编辑评审与"对不对"一致性校对合并为一个 `reviewer` 专用只读子 Agent。** 曾把"好不好"编辑评审定为主要角色经通用子 Agent 委派（理由"只需上下文隔离"），此判断漏算了它与一致性校对同样需要**工具层强制的 workspace 只读**（判据 (a)：评审只回意见/方向、绝不改稿，而通用委派会默认带 `write_file`/`edit_file`，只读全靠 prompt 约定）。且"好不好"与"对不对"**形状完全一致**：同样只读、同样读固定输入（文件/范围 + 章纲 + `project.md`）、同样以简报 + 落盘检查文件交付、同样绝不改稿，差别只在评审镜头（stance），而镜头由 brief 意图选载——正是 §2.1 落位第 4 条（conditional）= subSkill。因此二者合并为一个 `reviewer` 专用只读子 Agent（原 SA03 ConsistencyChecker 升格），三场景由 brief 意图决定：**好不好 / 对不对 / 两者皆做**。agent.md 正文只承载**公共评审协议**（身份 / 场景路由 / 输入输出契约 / 红线），两套镜头 `editorial-review`（好不好）与 `consistency-review`（对不对，承接原 S06 忠实度+质量两阶段）作为按意图选载的 subSkill 挂在 `creative/review/`，负向护栏 `restraint` 与 writer 同源共用（`creative/common/`）。**输入** = 文件/文件范围 + 意图 + 场景范围；**输出** = 简报 + 检查文件写入免审批虚拟区 `/large_tool_results/review-*.md`（路径 + 简报回传，SA02 可接文件路径或内联意见）。权限沿用 SA04 Researcher 同款——不设 blanket write-deny（否则 temp 检查文件写不了），红线限定只写 `/large_tool_results/`，workspace 写动作由审批层拦截（requires-review、不静默落盘），temp 写自动放行。**多分身并行评审为后期增强**（按视角裂分身的判据见 §2.3/04.3），初始为单分身按场景整体评审。
...
- **核心角色（subAgent，主 skill 写进 agent.md；A00 主控写进 system prompt）**：A00 主控（主 skill S01 task-routing 折叠进 A00 system prompt——每轮必用、单一消费者，不作独立挂载 skill）；SA01 Explorer（主 skill S02）；SA02 Writer（主 skill S05b）；SA03 Reviewer（主 skill S06 review：好不好+对不对+两者三场景，workspace 只读，产出 `/large_tool_results/` temp 检查文件；两套镜头 `editorial-review`/`consistency-review` 按意图选载）；SA04 Researcher（主 skill S08，共享 `common/`）。
...
| SA03 | Reviewer子agent | Reviewer | 三场景只读评审：好不好（编辑评审）/ 对不对（忠实度+质量一致性）/ 两者；产出简报+`/large_tool_results/` 检查文件 | A00 判定为评审类并委派（写后自动跑好不好=模式A；作者触发=模式B） | S06（含镜头 editorial-review、consistency-review） |
...
| S06 | 主skill | review（reviewer 公共评审协议，折叠进 agent.md） | 三场景只读评审：好不好（editorial-review 镜头）/ 对不对（consistency-review 镜头=忠实度 SS11+质量 SS12 两阶段）/ 两者；按 brief 意图选载镜头，产出简报+`/large_tool_results/` 检查文件 | 评审类（模式A 写后自动只跑好不好、模式B 作者触发任一场景） | editorial-review、consistency-review（含 SS11、SS12）、SS03、SS04、SS02 |
...
| —(镜头) | 子skill | editorial-review | 好不好评审镜头：一个粗粒度全视角技能（戏剧性/人物与声音/视角/行文/结构与节奏/主题/读者体验），只给方向不写细则 | reviewer 场景含好不好时选载（模式A 只此镜头/模式B 可含） | 共用 restraint（`creative/common`） |
| —(镜头) | 子skill | consistency-review | 对不对评审镜头 = 忠实度（SS11）+ 质量一致性（SS12）两阶段合一，两 verdict 分开产出 | reviewer 场景含对不对时选载（模式B、commit 提醒） | SS11、SS12 |
...
| 评审类 | 写稿后（模式A 自动只跑好不好）/ 审校期（模式B 作者触发好不好/对不对/两者，含局部大修与全稿定稿） | S06 review（reviewer 子agent） | editorial-review、consistency-review（=SS11+SS12）、SS03、SS04、SS02、(SS05 条件) | reviewer 子agent 委派（workspace 只读、隔离上下文、只回意见/落 temp 检查文件不改稿；全稿级可加读 `status.md`） |
...
| S06 review | 读 `manuscript/`、`outline/`、`characters/`、`worldbuilding/`；需要全局摘要时可读 `.iwriter/status.md`；写 `/large_tool_results/review-*.md`（检查文件，workspace 只读） | get_blocks（按核查颗粒度）、find_references、write_file（仅 `/large_tool_results/`） | editorial-review、consistency-review（=SS11+SS12） |
...
- **S06 review（SA03 Reviewer）**：workspace 只读的三场景评审——brief 意图决定跑哪个场景：**好不好**（选载 `editorial-review` 镜头：戏剧性/人物与声音/视角/行文/结构与节奏/主题/读者体验，只给方向）、**对不对**（选载 `consistency-review` 镜头：忠实度 SS11 + 质量 SS12 含 FR-5.1 时序/时间线维度，两 verdict 分开产出）、**两者**（两镜头都跑、出两张表）。输入可为作者提供的外部试读反馈（整理为分级问题清单，不裁决对错）；全稿宏观级超单批预算时由 A00 按卷/结构节点分批委派、跨批聚合（§2.14）。**产出**：每次评审把详细发现写入免审批虚拟区 `/large_tool_results/review-*.md`，回传简报 + 文件路径（SA02 可接文件路径或内联意见）。**触发与落盘**：模式A（SA02 写后 A00 自动跑好不好、一轮修订、临时不落盘）/ 模式B（作者显式触发任一场景）；是否把 temp 检查文件提升进 `process/review-findings.md` 是下游 A00 的动作（写工作区、经普通编辑审批）——全稿审校/外部反馈整理默认提议落盘、局部自查默认只在对话呈现。后续修订任务（S05 修订链路）可引用该清单，修复后顺带更新条目状态（经审批）。
...
机制结论：deepagents 技能加载以**目录为挂载单元**、多个源之间同名 last-wins；子 Agent 不继承主 Agent 技能（框架自动提供的 general-purpose 例外——S09 与 S10 提炼批次（SS17）通用委派的承重假设，实施前优先核验）；项目级 skills 目录 `{workspace}/.iwriter/skills` 固定挂各源列表末位，last-wins 即 FR-9.3"项目级自定义 > 内置默认"优先级裁决的机制实现。creative 实例结论（按 §2.1 三层分类）：**核心角色的主 skill（S02/S05b/S06/S08）折叠进各自 `subagents/{name}/agent.md`，不在 skills/ 下单独挂载**；**主要角色的主 skill 挂载**（`creative/main`：S01/S03/S04/S05a/S07/S10/S11；`creative/delegated`：S09，挂 A00 供 general-purpose 继承）；**评审镜头子技能挂 reviewer 专属池**（`creative/review`：`editorial-review` 好不好镜头 + `consistency-review` 对不对镜头，按 brief 意图选载）；**跨角色共用护栏** `restraint` 在 `creative/common`（writer 写时避、reviewer 好不好镜头审时抓，单一真源）；**SS01-SS17 是 subSkill、引用不挂载**（放共享文件池 `creative/reference/` 供相对路径引用、零拷贝，但不进任何执行体的 `skills` 源；过渡期暂挂载，随 §10 验证逐步归位）；技能目录名 = 技能 name（常规 kebab-case，不带 ID 前缀——编号是设计文档层索引，见 §2.2 总表）。完整目录树、挂载矩阵与现有旧技能的收编/下线映射见《04.3》§3。

...
设计要做的事情，是把"**从灵感到一部小说完稿**"这个完整创作过程中涉及的全部场景与流程（含每个环节的产出物），忠实地复刻到这个框架已有的四类积木上：**main agent（A00）+ sub agent（SA01-SA04）+ skills（S01-S11 主技能、SS01-SS17 子技能）+ tools**。设计的主体工作不是发明一套新的架构概念，而是回答"这个场景对应哪段流程、这段流程产出什么、该由哪类积木承担"——这个映射关系本身才是设计内容，具体映射结果见 §2；§2.2 附有从灵感到完稿的**小说创作流程图**，展示这个流程如何落到 A00/SA/S/SS 各角色上，那张是流程图，不是这里要给的整体架构图。
...
block:layers["本设计：分层实例化（自下而上）"]
  ...
  L3["Skills + subAgent 层：S01-S11 / SS01-SS17 + A00/SA01-SA04（§2.2-2.6）"]
...
按此重审，专用子 Agent 收敛为**三个**：SA02 Writer（写稿高频 + 块级编辑工具面）、SA03 Reviewer（工具层收窄的 workspace 只读 + 独立上下文避免确认偏差；合并"好不好"编辑评审与"对不对"一致性校对为一个只读评审执行体，详见下段）、SA04 Researcher（纯共用承包商，web 工具面 + 交付件落盘会话级 `/large_tool_results/`）。**原 SA01 Explorer 退役**：发散（生成有区分度的方向）是 A00 对话里的思考动作、不是独立执行体——只有"上下文隔离/固定契约"才构成设专用子 Agent 的理由，而发散既无需更窄工具面、也无需隔离上下文（方向的价值在被作者当场比较，不在落盘），因此降为 A00 载 `direction-ideation` 技能直接跑；探索到的"可行性查证"（史实/专业/世界规则）归 SA04 Researcher，不另设创意型分身。专用子 Agent 本身也不写死在代码里，而是以声明式定义文件动态装配（见 §2.10）——"身份定位、工具范围、输入输出契约、安全红线"这四类 prompt 内容恰好就是一个小 markdown 文件的体量。曾设想的 WritingStyle 专用子 Agent 不满足任何专用化判据：它不需要更窄的强制工具面（写入 `styles/` 之外的路径已有 `interruptOn` 审批兜底）、不需要不同模型、频率极低（一个项目通常只提炼一两次风格）；它唯一的真实诉求是隔离大段范文文本，通用子 Agent 委派即可满足。因此风格提炼定为 **S09 技能 + 通用子 Agent 委派**，不设专用执行体。反向提炼同样不设专用执行体，但执行途径不同：S10 定为 **A00 直接执行的导入编排主技能**——整备阶段（SS16）必须与作者交互确认章节边界，而子 Agent 内没有对话通道；只有其提炼批次（SS17）需要隔离大段正文上下文，经通用子 Agent 委派（§2.5、§6）。
...
关键设计点（完整契约在《04.3》§1 S06）：agent.md 正文承载**公共评审协议 + 统一输出契约**（一套五类输出 + 四级优先级，只在 agent.md 定义、三镜头不重复），三镜头用同一四段式模板；核心原则**分阶段递减干预**（越近定稿越收方向性干预）；**语法校对归 App 的 proofread（LanguageTool）、不归 reviewer**；权限沿用 SA04 Researcher 同款（只写 `/large_tool_results/`、workspace 只读靠审批层）；多分身并行为后期增强，初始单分身。
...
因此下表"分类"列中的"主 skill / 子 skill"是沿用的文档索引词：核心角色的主 skill（S01/S02/S05b/S06/S08）实际折叠在 agent.md（A00 的 S01 折叠进 system prompt），其余主 skill 为主要角色挂载单元，"子 skill"一律是 subSkill。
...
- **批间传递靠落盘中间产物 + A00 编排聚合**，不靠子 Agent 会话延续、批与批的子 Agent 之间不直接通信：
  - **S10** 逐批产出两类中间产物——章纲草稿直接写 `outline/ch{NNN}-outline.md`（标 `status: draft`，经审批，本身就是交付的一部分）；角色/设定层的行为证据与中间归纳落会话级 `/large_tool_results/`（免审批虚拟区，复用 SA04 交付件通道，见《04.1》§6；执行提炼批次（SS17）的通用子 agent 因此开放 `write_file`，**仅限该虚拟区**）。终批完成后 A00 追加一次**聚合委派**：读全部批次中间产物与已落盘章纲草稿（不重读全稿正文），反推总纲结构节点与伏笔总表、合并去重角色/设定候选写 `exploration/`。
  - **S06 review** 每批把详细发现写入 `/large_tool_results/review-batch-{n}.md`（免审批虚拟区），并以固定格式响应文本回传 A00 简报 + 该批文件路径（宏观级结论紧凑，响应文本装得下），回报中单列**"跨批线索"段**（未回收伏笔、未闭合时间线锚点、批末角色状态快照）；A00 把前批跨批线索摘要随下一批 brief 前向下达，终批后读各批 temp 文件跨批聚合，两 verdict 仍分开产出。**reviewer 对 workspace 保持只读**——中间发现只落 `/large_tool_results/`（同 SA04 交付件通道），绝不写工作区对象。

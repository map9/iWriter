...
flowchart LR
  ...
  subgraph 技能层["技能层：主 Skill（每个创作场景一个）"]
    ...
    S02["ideation-outline-flow（构思循环 playbook：发散·比较·收敛）"]
...
| S02 | 主skill（A00 直接执行·构思 playbook） | ideation-outline-flow | 构思阶段元循环指挥（发散·比较·收敛 + 相互讨论平衡阀 + 阶段过渡），路由到发散/比较/收敛技能 | 构思类（主题/设定/大纲环节） | direction-ideation、idea-review、outline-review（收敛转 S03/S04/S11） |
...
| --- | --- | --- | --- | --- |
| 构思类（主题/创意环节） | 前期筹备 | S02 ideation-outline-flow（playbook）→ idea-review（比较）/ direction-ideation（发散）/ S11（收敛） | SS13、SS09、SS06、SS07 | 构思模式（发散·比较·收敛） |
...
| 初始化类 | 前期筹备（进入之前，从零新建工程） | S11 | SS01（骨架创建另引 SS02/SS03/SS04 字段标题） | 最小装配（空工程无既有对象可读；premise 引导可转构思循环（ideation-outline-flow）） |
...
| S02（ideation-outline-flow） | 收敛写 `project.md`/`worldbuilding/`/`characters/`/`outline/`（经 S03/S04/S11）；发散可选暂存 `exploration/` | 块级读写 + create_document | SS13、idea-review、outline-review |
...
- **S02（ideation-outline-flow，A00 直接执行）**：构思阶段的元循环 playbook。每轮先定位"在哪个环节（主题/设定/大纲）× 哪一拍（发散/比较/收敛）"，再按拍执行——发散载 `direction-ideation`（否定显而易见版本、承重轴分叉、只描述差异不评优劣）；比较载 `idea-review`/`outline-review`（按发展阶段整体评审、给判断/方向/方案/验证任务）；收敛经 S11/S03/S04 转录进正式对象。相互讨论平衡阀：主动抛框架点、不代作者裁决、每轮只展开 2–4 个最重要的点其余挂开放待决。候选未确认不进正式对象。

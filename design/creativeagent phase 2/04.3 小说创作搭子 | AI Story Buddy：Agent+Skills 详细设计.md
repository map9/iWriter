...
1. 读取或遵循 `.iwriter/creative/agent.md` 中的项目级协作规则：作者成熟度、干预强度、风险偏好只影响工作方式、风险阈值、解释深度、澄清频率与上下文恢复策略，不提供剧情事实；
...
5. 风险校验：落地目标是正式对象还是候选区，风险越高越应先澄清；可按 `agent.md` 的风险偏好调整说明颗粒度与澄清阈值，但不得降低 §5 审批红线；
...
技能挂载机制（目录即挂载单元、同名 last-wins、渐进披露三层）见《04.1 全能创作搭子｜AI Writing Buddy：脚手架 详细设计》§2。
...
按 03 §2.1 的原则，执行体 prompt 只承载四类内容：**身份定位、工具范围（frontmatter `tools` 承载，prompt 正文不重复罗列）、输入输出契约、安全红线**；流程逻辑一律在 skill 里，prompt 通过指针引用主 skill。以下是各执行体 prompt 必须包含的关键点，作为编写 `agents/{domain}/{sa-slug}.md` 正文与 A00 system prompt 的检查清单：

**A00 主控 Agent**

- 身份定位：Creative Domain 主控——任务理解、S01 路由、结果收束；直接执行设定类（S03）、大纲类（S04）、重构诊断（S07）与记录类（SS15 轻量追加），其余委派。
- 输入输出契约：委派 brief 必含任务类别与目标对象路径；S05 展开链路必须附已批准计划与场景清单；S08 必含 question + scope；S10 必含提炼范围与章节区间。子 Agent 回报以最终响应文本呈现，不使用结构化提交工具（issues#1）。
...
**SA04 Researcher**（`agents/common/` 池成员；creative 同名覆盖版追加 `creative/research` 技能池挂载）

- 身份定位：现实细节与专业背景查证执行体。
- 工具面：`web_search` / `fetch_url` + 文档读取；creative 覆盖版可写 `exploration/research-{topic}.md`。
- 输入输出契约：brief 必含 `question` 与 `scope`；产出区分"观察到的证据"与"解读"，标注来源、置信度与信息缺口。
- 安全红线：不编造事实与来源；不做文档写入以外的文件操作。
- skill 指针：S08。

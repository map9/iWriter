...
> 本文档由《03. 小说创作搭子｜AI  Story Buddy：概要设计》§2.7 拆出，承载专用工具的完整论证、被取消能力的替代方式与现有工具的处置对照。审查方法与判据、通用工具面清单（24 个）、`git_restore` 缺口、去数据库化原则与 interruptOn 审批集合结论保留在 03 §2.7；块级读写协议（通用读写工具的行为层设计）见《04.1 全能创作搭子｜AI Writing Buddy：脚手架 详细设计》§5。**本文中的 §x 章节引用如无特别说明，均指《03. 概要设计》的章节编号。**

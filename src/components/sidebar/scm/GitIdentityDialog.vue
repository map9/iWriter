  ...
  >
    <div class="w-96 max-w-[90vw] overflow-hidden rounded-box border border-base-300 bg-base-100 shadow-2xl">
      <div class="flex items-center justify-between border-b border-base-300 px-4 py-3">
        ...
        <button class="btn btn-ghost btn-square btn-sm px-2" @click="gitStore.cancelIdentity()"><IconX class="icon-xs" /></button>
      </div>
      <form class="flex flex-col gap-3 px-4 py-4" @submit.prevent="save">
        ...
        <label class="flex flex-col gap-1">
          ...
          <input v-model="name" type="text" class="input input-sm h-7 w-full" required />
        </label>
        <label class="flex flex-col gap-1">
          <span class="text-xs text-base-content/60">{{ t('sourceControl.identity.email') }}</span>
          <input v-model="email" type="email" class="input input-sm h-7 w-full" required />
        </label>
        ...
        <div class="flex justify-end gap-2 pt-1">
          <button type="button" class="btn btn-ghost btn-sm" @click="gitStore.cancelIdentity()">{{ t('common.cancel') }}</button>
          <button type="submit" class="btn btn-primary btn-sm" :disabled="!name || !email">{{ t('sourceControl.identity.save') }}</button>
        </div>
      </form>
    </div>
  </div>

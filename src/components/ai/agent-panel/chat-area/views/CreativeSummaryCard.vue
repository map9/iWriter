<template>
  ...
  >
    ...
    >
      <div class="flex w-3.5 shrink-0 items-center justify-center text-sm leading-none">
        {{ t('agentPanel.creativeSummary.icon') }}
      </div>
    ...
    <div v-if="expanded" class="space-y-2 border-t bg-base-100 px-2 py-2" :class="bodyClass">
      ...
      <div class="space-y-1">
        ...
        >
          ...
          <div v-if="item.failureMessage" class="mt-1 text-2xs text-error-content">
            {{ item.failureMessage }}
          </div>
        </div>
      </div>
...
const summaryLabel = computed(() => {
  ...
  const parts: string[] = []
  if (appliedTotal > 0) parts.push(t('agentPanel.creativeSummary.summary.appliedChanges', { count: appliedTotal }))
  if (props.result.skipped > 0) parts.push(t('agentPanel.creativeSummary.summary.skipped', { count: props.result.skipped }))
  if (props.result.failedToApply > 0) parts.push(t('agentPanel.creativeSummary.summary.failed', { count: props.result.failedToApply }))
  if (!parts.length) parts.push(t('agentPanel.creativeSummary.summary.totalHandled', { count: props.result.total }))
  return parts.join(' · ')
})
...
function stateLabel(state: CreativeRoundResultState): string {
  switch (state) {
    case 'applied': return t('agentPanel.creativeSummary.states.applied')
    case 'applied_edited': return t('agentPanel.creativeSummary.states.appliedEdited')
    case 'failed_to_apply': return t('agentPanel.creativeSummary.states.failedToApply')
    default: return t('agentPanel.creativeSummary.states.skipped')
  }
}

...
RETURN: one block per scenario, each with its own verdict; findings graded BLOCKING/MAJOR/MINOR/OPTIONAL with locations; what must NOT be changed; the findings-file path under /large_tool_results/. Edit nothing.
""")
\`\`\`
...
- **A negative search result proves nothing.** The search tool is literal, not pattern-based — one term per query, and re-verify before concluding that anything is absent.

...
- 只有任务依赖当前标签、选区、光标、dirty 状态或 open tabs 时，才调用 \`get_editor_state\`；界面状态可能变化后重新获取，不凭历史状态推断当前界面。
...
- 一次模型响应只能提交一种需要审批的写操作家族：①块编辑与 \`create_document\`；②文件系统变更；③创作确认、导入与 Git 操作。提交后停止并等待审批，不与另一家族并行调用。普通读取、搜索和分析工具不受此限制。

...
- 只有任务依赖当前标签、选区、光标、dirty 状态或 open tabs 时，才调用 \`get_editor_state\`；用户切换标签或界面状态可能变化后重新获取，不凭历史状态推断当前界面。
- 工作区对象使用相对路径，host 会依据隐藏的 runtime workspace 解析；不要为了调用工具探测、拼接或询问工作区绝对根路径。

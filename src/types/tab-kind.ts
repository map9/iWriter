...
export interface TabKindDescriptor {
  ...
  // 注：Page 组件的懒加载映射放在渲染层（见 MainView / tabPageComponents），
  // 不放这里——本文件被 electron 主进程 tsc 编译，不能引用 .vue。
  ...
  displayName: string
  /**
   * 覆盖标题栏图标；返回 null / 不提供 → 沿用默认（文件型按扩展名）。
   * 非文件型（如 diff）在此提供 kind 专属图标。
   */
  icon?: (tab: FileTab) => Component | null

...
description: 对 Markdown 小说工作区执行创建、定位、读写、删除、校验或对象路由时使用；定义紧凑文件树、稳定 ID、唯一事实源、卡片流转与最小读取方式。
---
...
- 卡片用一个原子事实句；故事线阶段、结构节点和场景各用一个因果句。
- 简单人物或设定用一个核心句，必要时最多三条补充。
- 承重人物或设定按对应模板选用功能行，默认不超过六条；没有有效内容的行省略。
- 一行合并相互依赖的事实；独立且可单独引用的事实建立稳定 ID 子条目。
- 删除定义、方法说明、同义复述和无证据推测；不得为满足长度截断已确认事实。

...
description: 项目对象的非主流程结构规范；仅在当前主 Playbook 已完成任务定位并进入创建、更新、迁移、删除或结构校验阶段，或确实无法确定对象归属与路径时加载；定义紧凑文件树、稳定 ID、唯一事实源、卡片流转与最小读取方式。
---
...
规定项目对象的位置、引用、修改与校验。创作语义由对应任务模块验收；项目文件不保存分析方法。
...
- 当前伤势、位置、持有物、知情范围、承诺和关系胜负等实际状态从正文与相邻章纲判断，不在稳定人物或世界对象中另存一份。会约束目标章写作、但不能从相邻正文结尾直接判断的状态，在目标章纲中只写本章需要的开始条件。
...
- 核心句默认不超过 120 个中文字符或 60 个英文单词。
- 补充行默认不超过 100 个中文字符或 50 个英文单词。
- 卡片默认用一个原子事实句；故事线阶段、结构节点和场景默认各用一个因果句。
- 简单人物或设定用一个核心段，不补空白信息。
- 主要人物或重要设定默认最多增加六条正文会使用的自然事实句；不设固定字段。
- 一行合并相互依赖的事实；独立且可单独引用的事实建立稳定 ID 子条目。
- 默认预算无法无损表达时，增加最少必要的句子，或把独立事实拆成稳定 ID 子条目；不得为了凑行数把无关事实塞进同一句。
- 删除定义、方法说明、同义复述和无证据推测；不得为满足长度预算截断已确认事实或关键因果。

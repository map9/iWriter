...
5. **Lazily create the required object skeletons** — `worldbuilding/worldbuilding.md`, `characters/characters.md`, `outline/master-outline.md`, each a field-title skeleton per its schema (load `worldbuilding-schema` / `character-schema` / `outline-schema`), the outline marked `status: 草稿中` (draft). Write each with `create_document`, passing its subfolder as the absolute `directory` and `open_in_editor=false` (on-disk scaffold, no tab) — see `document-block-tools` for the call contract. Do not pre-create any optional object (lazy-creation). Content is filled later by the ideation/authoring flows.

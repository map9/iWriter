...
Initialization must be author-confirmed, never silent (FR-1.6). Do not write `project.md` if the premise's four elements do not hold. Do not invent work facts. The version field never auto-advances. Do not skip the `git_init` proposal (step 3) when there is no `.git` — offer it and let the author decide.

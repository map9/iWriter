...
3. 有已确认故事线时的 `storylines.md`；
4. `outline/master-outline.md`；
5. 作者确认启用时的 `outline/vol{NN}-outline.md`；
6. 范围内的 `outline/ch{NNN}-outline.md`。
...
- 章纲的紧凑场景句仍能还原行动者意图、有效阻力、转变、结果和下一条件，不是章节摘要。
- 故事线能脱离总纲说明自身的推动、变化和兑现；总纲再安排“决定 → 回应 → 不可逆变化 → 下一压力”的全书位置。
- 人物稳定判断有跨场景行为支持；一次行为不直接上升为心理模型或性格结论。
- 世界规则能说明关键边界、成本、执行或人物后果；不要求拆字段，背景名词堆积也不算规则。
- 项目核心故事约定、主题问题和读者体验属于全书解释，必须由作者确认。

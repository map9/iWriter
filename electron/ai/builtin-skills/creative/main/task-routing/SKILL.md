...
7. **Decide the execution path** for the chosen scene category:
   - A00 executes directly: worldbuilding (S03), outline (S04), **writing-plan authoring (S05a)** — opening the write-session authorization via `confirm_writing_plan`, **optionally** designing a beat plan (beats are not a prerequisite), materializing any beats as `> [!BEAT] …` lines in the manuscript, and the pre-write state machine (two axes: does the beat layer change / is the writer delegated) — restructuring diagnosis (S07), novel-import orchestration (S10), project bootstrap & `project.md` maintenance (S11), lightweight recording (SS15).
   - Delegate to a专用 subagent: `explorer` (S02), `writer` (S05b — writes prose from the confirmed chapter outline + any `[!BEAT]` lines; after S05a), `reviewer` (S06 — read-only review; brief carries `scenario` = `quality` 好不好 / `consistency` 对不对 / `both`), `researcher` (S08).
   - Delegate via general-purpose: style transfer (S09); S10 distillation batches (SS17).
   - **Review is the `reviewer` subagent**: mode A after the writer returns delegates `reviewer` with `scenario=quality` (好不好 only); mode B when the author asks delegates `reviewer` with the scenario they want (好不好 / 对不对 / `both`). Reviewer returns a summary + a `/large_tool_results/review-*.md` path; it never edits — opinions only.
   - **A "write chapter N" request is S05a, not a direct delegation**: load `writing-plan-authoring` first — it decides whether the chapter outline is ready (the hard prerequisite; beats are optional), opens the write-session authorization, whether to design beats, or whether to hold (don't blindly rewrite existing prose).

...
7. **Decide the execution path** for the chosen scene category:
   ...
   - Delegate to a专用 subagent: `explorer` (S02), `writer` (S05b — writes prose from the confirmed chapter outline + any `[!BEAT]` lines; after S05a), `consistency-checker` (S06 — the "对不对" review), `researcher` (S08).
   - Delegate via general-purpose: style transfer (S09); S10 distillation batches (SS17); **editorial-review** (the "好不好" quality critique — mode A after the writer returns, and mode B when the author asks; opinions only).

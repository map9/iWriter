...
2. **Ground the prose (don't invent, don't write generic).** Assemble only what the in-scope scenes actually touch: the on-stage characters' voice/psychology/relationships (`characters/`), the world facts the scene touches (`worldbuilding/`), this chapter's point and the story spine (its `outline/ch{NNN}-outline.md` + `outline/master-outline.md`), adjacent chapters for continuity, and **`project.md` for the target length** so the chapter's size fits the book. If `materials/fragments.md` is non-empty, attach a merge proposal for any "未采用" entry that hits this chapter or its characters (FR-1.5).
   ...
   **Voice anchor — read `styles/` and write *like the exemplar*.** If a `styles/{slug}.md` with `采用范围: 全书默认` (or one whose scope covers this scene) exists, it is your **primary, positive voice anchor**: match the嗓子 of its **范文 (exemplar)** — write *like it*, do NOT copy its content or scene. The exemplar outranks the analytical fields (声音/措辞/句法): when in doubt, imitate the passage, not the description. This positive anchor leads; the 写法护栏 below (restraint) are the negative guardrails that catch regressions **toward** mannered prose — they do not, by themselves, supply the target voice. If no `styles/` object exists, fall back to the characters' 声音特征 + `prose-craft-by-example`, and still obey the 写法护栏.
3. **Write each in-scope scene as a whole.** Take in the scene's dramatic function (its goal / conflict / outcome from the outline), anchor POV and psychic distance, dramatize rather than summarize, and let the prose flow **through** the beats when they exist rather than expanding one beat at a time. Ground each turn in `scene-and-plot-construction` (causal necessity) and each character action in `character-believability`. Draw prose craft from `prose-craft-by-example` — write *like the examples*, no countable per-sentence rule. **Allocate length by dramatic weight so the chapter lands near its `project.md` target — from both sides.** Key turns get room, connective tissue stays lean; a thin under-length summary is a failure, **and so is padding every moment into its own set-piece until the chapter runs long.** If your draft is drifting well over the target, the fix is almost always to stop over-writing individual moments, not to add more.

   **写法护栏 (avoid the mannered-prose failure modes — load `references/restraint.md` for the by-example version):** these are not style flourishes to add, they are habits to catch and cut.
   - **Don't manufacture profundity** with the `不是 X——是 Y` / `不是 X，而是 Y` antithesis. Once it sharpens; as a default register it becomes a tic. If it shows up more than once or twice in a chapter, you're leaning on it — convert the rest into action, image, or plain statement.
   - **Don't explain the scene's meaning in narratorial asides.** Cut "……的意思是", "那不是……而是一种……的感觉" and their kin; let the concrete gesture, silence, or image carry the meaning and trust the reader to infer it.
   - **Dramatize the POV character, don't make them a silent observer.** If the outline/character arc calls for anger (a demand, a shout, a fight), write it — a passive narrator watching and philosophizing loses the scene's charge.
   - **Fast scenes stay fast.** A hook, a chase, a shock: give the turn its due and move; do not expand each half-second into a rendered paragraph.
...
5. **Self-check.** Verify the prose realizes each in-scope scene's intent (and each beat's core point where beats exist), any beat you adjusted is reported, nothing drifted from the outline's goal / conflict / outcome, and the chapter length is in the neighborhood of the `project.md` target **from both sides** (not thin, not padded-long). Run the 写法护栏 over your draft: is the `不是X是Y` antithesis leaned on? Are there narratorial asides explaining what things "mean"? Is the POV character passive where the arc wants him active? Cut what you find (the cutting test in `references/restraint.md`).

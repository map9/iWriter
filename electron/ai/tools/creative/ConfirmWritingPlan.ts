...
export function buildConfirmWritingPlanTool(): StructuredTool {
  return tool(
    ...
    },
    {
      ...
      schema: z.object({
        plan: z.string().describe('Complete author-readable Markdown writing plan at BEAT granularity: for each confirmed outline scene, the 2-5 beats it breaks into, each a one-line core point (核心点) that is causally necessary. This is beat-level intent (what to write), not line-by-line wording. On approval you write these beats into the manuscript as `> [场景-{N}-节拍-{M}] 核心点` sentinels; the writer expands the prose beneath each.'),

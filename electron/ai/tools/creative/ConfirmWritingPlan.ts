...
// after approval the main agent (the drafting-playbook skill) MATERIALIZES them as
  ...
  return tool(
    ...
    },
    {
      ...
      schema: z.object({
        ...
        target_files: z
          .array(z.string().refine(value => path.isAbsolute(value.trim()), 'target_files must contain absolute paths.'))
          .optional()
          .describe('Manuscript chapter file(s) this plan authorizes for writing (usually one; a multi-chapter restructure plan lists several). On approval these become the write-session scope: block edits to them (your sentinel writes and the writer\'s prose) auto-accumulate. Chapter outlines and other objects are NOT included — they always go through normal review.'),
      }),
    },

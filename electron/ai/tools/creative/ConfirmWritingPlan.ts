...
// model (§5 "先批意图→逐块自动累积→整章 diff 终审"). Approving it opens the write-session before
// drafting. Generalized to "write-session authorization (可含 beat)": beats are OPTIONAL.
//
...
// after approval the main agent (S05a writing-plan-authoring) MATERIALIZES them as
// `> [!BEAT] [场景-{N}-节拍-{M}] 核心点` in the target chapter (new chapter via create_document,
...
export function buildConfirmWritingPlanTool(): StructuredTool {
  return tool(
    ...
    },
    {
      ...
      description: 'Ask the author to approve or edit a WRITE-SESSION AUTHORIZATION before drafting a chapter or a large rewrite/restructure. Approval opens the write-session (block edits to target_files auto-accumulate) and records the confirmed intent. Beats are OPTIONAL: the plan may be a beat skeleton OR a one-line writing intent when writing straight from the confirmed chapter outline. When it carries beats, AFTER approval you (the main agent) materialize them as `> [!BEAT] …` lines in the target manuscript file(s); the writer reads them from the file — do not transcribe the plan into the writer\'s brief.',
      schema: z.object({
        plan: z.string().describe('EITHER a beat skeleton OR a one-line writing intent (beats are optional). Beat skeleton: each beat is its own GFM Alert `> [!BEAT] [场景-{N}-节拍-{M}] 核心点`, and **consecutive beats MUST be separated by a blank line** (adjacent `> [!BEAT]` lines with no blank line collapse into one alert box on disk). `[!BEAT]` is the fixed marker, the `[场景-N-节拍-M]` coordinate is REQUIRED here (agent-authored beats always carry it), scenes separated by a `* * *` break, each 核心点 a single causally-necessary sentence with no explanatory tail or style instruction (beat-level intent, not line-by-line wording); on approval you copy these VERBATIM into the manuscript and the writer expands the prose beneath each. Writing intent (no beats): a short line like "从已确认章纲写第 3 章，范围：全部场景" — the writer then writes straight from the chapter outline scenes.'),

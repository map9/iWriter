...
2. **Ground** (minimal necessary — don't bulk-read the project). Take in only what the in-scope scenes touch: on-stage characters (`characters/`), world facts the scene uses (`worldbuilding/`), this chapter's point and the spine (its outline + `outline/master-outline.md`), adjacent chapters for continuity, and **`project.md` for the target length**. To look up a specific established fact (a catchphrase, an item, a term), search `characters/` / `worldbuilding/` — encouraged.
   - **Voice exemplar — `styles/{slug}.md` (the PRIMARY positive target when one exists).** If a style object applies to this scene (its scope is whole-book or covers the scene), match the **voice of its exemplar prose** — its syntax, tone, and image-making. The exemplar outranks its own analytical fields and any generic craft. **Take only its voice, never its content**: do NOT reuse the exemplar's scene, plot, objects, or specific images — and when the exemplar's scene happens to resemble the one you're writing, the more it resembles, the harder you must invent your own concrete material. An exemplar is an anchor, **never a precondition**: with no style object you write from the characters' voice traits plus the technique references below, at full quality — never stall, hedge, or ask for one.
   - **Lived-in material beats observed material.** The details that make a scene real are the ones only *these* people in *this* history could produce — a habit, an old grievance, a thing one of them ruins for the other — not what any observer in the room would notice. You already have this material: it is in the character files, the world facts, and earlier chapters. Mine it before inventing atmosphere. There is no quota; a scene that needs none takes none.
...
6. **Deliver.** Prose (and any beat you adjusted) through the block-edit tools with `file_path=<TARGET>` and `expected_current_content` on every edit/delete — or, for the new chapter you were told doesn't exist, one `create_document` (content + `directory`). Never emit prose in your response text.
   - **Load `document-block-tools` before your first block read or edit** — not optional, and not something to skip because the tools look obvious. It is the single source for the `{b:n}` marker rule (the marker is an address, never content — strip it from `expected_*` and from everything you write), for how a batch of edits shares one read, and for what to do when an edit reports a content mismatch. Most repeated mismatch failures are a marker or a wrong-granularity copy in `expected_*`, not a changed file, and re-reading alone will never fix them.
...
- **Technique references (on demand, by need).** From the **`prose-craft-by-example`** skill, read the one `references/*.md` that fits the weakness in front of you — write *like the examples*, no countable per-sentence rule:
  - the scene turns on a shock, a reversal, a blow → `impact-rendering`
